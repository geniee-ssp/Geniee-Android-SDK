...
public class AdAdapter {
    ...
    public AdAdapter(Activity activity, String unitId) {
        mActivity = activity;
    ...
    public void buildAdLoader() {
        ...
        // 端末サイズを越えるとAdが呼ばれないため調整する
        int windowDpSizeX = (int) convertPxToDp(point.x) - 5;
        int windowDpSizeY = (int) convertPxToDp(point.y) - 5;
        AdSize adSize = new AdSize(windowDpSizeX, windowDpSizeY);

        //RequestするAdSize取得
        if (isTablet()) {
            mRequestAdSize = AdSize.MEDIUM_RECTANGLE;
        } else {
            mRequestAdSize = new AdSize(windowDpSizeX, windowDpSizeY);
        }

        mAdView.setAdSize(mRequestAdSize);
